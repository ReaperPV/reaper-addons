...
export default function RunSplits() {
    ...
    const editGui = new ScalableGui(data, data.betterSplits).setCommand("moverunsplits")

    const bestSplits = new PogObject("ReaperAddons", {}, "data/bestSplits.json") // {"FLOOR": [seg1ms, seg2ms, seg3ms, ...]}
    ...
    const renderGui = () => {
        ...
        // The segment names
        Renderer.drawString(segmentNameCol, 0, 9, true)
        ...
        Object.values(splitTimes).forEach((time, i) => {
            ...
            Renderer.drawString(time, timeStartX + textWidthOffset, (i+1)*9, true)
        })
        ...
        Object.entries(currRunSplits).forEach(([segment, info], i) => {
            ...
            const color = diffFromBest < 0 ? "&a" : diffFromBest > delta * 0.1 ? "&c" : "&6" // Gold if PB, green if not, yellow if >10% from best
            const toDraw = `${color}(${prefix}${formatTime(Math.abs(diffFromBest), 2)})`

            Renderer.drawString(toDraw, timeDiffStartX + timeDiffPadding, (i+1)*9, true)
        })
    ...
    register("tick", () => {
        if ((!Settings.runSplits || !Dungeon.floor) && !editGui.isOpen()) {
            cleanUp()
            return
        }
